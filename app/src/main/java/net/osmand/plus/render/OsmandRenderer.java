...
public class OsmandRenderer {
	 ...
	 */
	public void generateNewBitmapNative(RenderingContext rc, NativeOsmandLibrary library, NativeSearchResult searchResultHandler, Bitmap bmp,
		...
		if (rc.width > 0 && rc.height > 0 && searchResultHandler != null) {
			...
			try {
				...
				// If res.bitmapBuffer is null, it indicates that rendering was
				// done directly to
				// memory of passed bitmap, but this is supported only on
				// Android >= 2.2
	...
	public void generateNewBitmap(RenderingContext rc, List<BinaryMapDataObject> objects, Bitmap bmp, RenderingRuleSearchRequest render,
					...
					if (icon.y >= 0 && icon.y < rc.height && icon.x >= 0 && icon.x < rc.width) {
						...
						if (((ind >> b) & 1) == 0) {
							...
							if (rc.getDensityValue(1) != 1) {
								...
								cv.drawBitmap(ico, null,
										new RectF(left, top, left + rc.getDensityValue(ico.getWidth()), top + rc.getDensityValue(ico.getHeight())),
										paintIcon);
							} else {
		...
		if (render != null) {
			...
			for (int i = 0; i < sz; i++) {
				...
				for (int j = 0; j < o.getTypes().length; j++) {
					// put(orderMap,
					// BinaryMapDataObject.getOrder(o.getTypes()[j]), sh + j,
					// init);
	...
	protected void drawObj(BinaryMapDataObject obj, RenderingRuleSearchRequest render, Canvas canvas, RenderingContext rc, int l, boolean renderText,
			boolean drawOnlyShadow, int type) {
		rc.allObjects++;
	...
	private void drawPoint(BinaryMapDataObject obj, RenderingRuleSearchRequest render, Canvas canvas, RenderingContext rc, TagValuePair pair,
			boolean renderText) {
		if (render == null || pair == null) {
			return;
		}
	...
	private void drawPolyline(BinaryMapDataObject obj, RenderingRuleSearchRequest render, Canvas canvas, RenderingContext rc, TagValuePair pair,
			int layer, boolean drawOnlyShadow) {
		if (render == null || pair == null) {
			return;
		}
