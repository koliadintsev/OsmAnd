...
//					log.info("Draw with layers " + (System.currentTimeMillis() - time));
	...
	@SuppressLint("WrongCall") private void drawOverMap(Canvas canvas, RectF latlonRect, RectF tilesRect, DrawSettings drawSettings) {
		...
		try {
			...
			//Error in restoring state, lets save it
			canvas.save();
		}
		if (showMapPosition) {
