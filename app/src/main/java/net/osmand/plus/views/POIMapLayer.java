...
public class POIMapLayer extends OsmandMapLayer implements ContextMenuLayer.IContextMenuProvider {
	...
	private Paint point;
	private OsmandMapTileView mapTilView;
	...
	public void getAmenityFromPoint(PointF point, List<? super Amenity> am) {
		if (objects != null) {
			int ex = (int) point.x;
			int ey = (int) point.y;
			int compare = getRadiusPoi(mapTilView.getZoom());
			int radius = getRadiusPoi(mapTilView.getZoom()) * 3 / 2;
			try {
				for (int i = 0; i < objects.size(); i++) {
					Amenity n = objects.get(i);
					int x = mapTilView.getRotatedMapXForPoint(n.getLocation().getLatitude(), n.getLocation().getLongitude());
					int y = mapTilView.getRotatedMapYForPoint(n.getLocation().getLatitude(), n.getLocation().getLongitude());
	...
	public boolean onSingleTap(PointF point) {
		...
		if (!am.isEmpty()) {
			...
			AccessibleToast.makeText(mapTilView.getContext(), res.toString(), Toast.LENGTH_SHORT).show();
			return true;
		}
		return false;
	}
	...
	private StringBuilder buildPoiInformation(StringBuilder res, Amenity n) {
		String format = OsmAndFormatter.getPoiSimpleFormat(n, mapTilView.getContext(), mapTilView.getSettings().USE_ENGLISH_NAMES.get());
		res.append(" " + format);
		if (n.getOpeningHours() != null) {
			res.append("\n").append(mapTilView.getContext().getString(R.string.opening_hours)).append(" : ").append(n.getOpeningHours()); //$NON-NLS-1$ //$NON-NLS-2$
		}
		if (n.getPhone() != null) {
			res.append("\n").append(mapTilView.getContext().getString(R.string.phone)).append(" : ").append(n.getPhone()); //$NON-NLS-1$ //$NON-NLS-2$
		}
		if (n.getSite() != null && n.getType() != AmenityType.OSMWIKI) {
			res.append("\n").append(mapTilView.getContext().getString(R.string.website)).append(" : ").append(n.getSite()); //$NON-NLS-1$ //$NON-NLS-2$
		}
		return res;
	}
	...
	public void onDraw(Canvas canvas, RectF latLonBounds, RectF tilesRect, DrawSettings nightMode) {
		...
		if (mapTilView.getZoom() >= startZoom) {
			objects.clear();
			resourceManager.searchAmenitiesAsync(latLonBounds.top, latLonBounds.left, latLonBounds.bottom, latLonBounds.right, mapTilView.getZoom(), filter, objects);
			int r = getRadiusPoi(mapTilView.getZoom());
			for (Amenity o : objects) {
				int x = mapTilView.getRotatedMapXForPoint(o.getLocation().getLatitude(), o.getLocation().getLongitude());
				int y = mapTilView.getRotatedMapYForPoint(o.getLocation().getLatitude(), o.getLocation().getLongitude());
				...
				if (id != null) {
					Bitmap bmp = RenderingIcons.getIcon(mapTilView.getContext(), id);
					if (bmp != null) {
			...
			if (mapTilView.getSettings().SHOW_POI_LABEL.get()) {
				TIntHashSet set = new TIntHashSet();
				for (Amenity o : objects) {
					int x = mapTilView.getRotatedMapXForPoint(o.getLocation().getLatitude(), o.getLocation().getLongitude());
					int y = mapTilView.getRotatedMapYForPoint(o.getLocation().getLatitude(), o.getLocation().getLongitude());
					int tx = mapTilView.getMapXForPoint(o.getLocation().getLongitude());
					int ty = mapTilView.getMapYForPoint(o.getLocation().getLatitude());
					String name = o.getName(mapTilView.getSettings().USE_ENGLISH_NAMES.get());
				...
				public void onContextMenuClick(int itemId, int pos, boolean isChecked, DialogInterface dialog) {
					if (itemId == R.string.poi_context_menu_call) {
						try {
							...
							mapTilView.getContext().startActivity(intent);
						} catch (RuntimeException e) {
							log.error("Failed to invoke call", e); //$NON-NLS-1$
							AccessibleToast.makeText(mapTilView.getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
						}
	...
	private void showDescriptionDialog(Amenity a) {
		Builder bs = new AlertDialog.Builder(mapTilView.getContext());
		bs.setTitle(OsmAndFormatter.getPoiSimpleFormat(a, mapTilView.getContext(), mapTilView.getSettings().USE_ENGLISH_NAMES.get()));
