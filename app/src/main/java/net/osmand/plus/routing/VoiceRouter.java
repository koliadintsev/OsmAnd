...
public class VoiceRouter {
	...
	// default speed to have comfortable announcements (if actual speed is higher than it would be
	// problem)
	// Speed in m/s
	...
	public void updateAppMode() {
		// turn prompt starts either at distance, or if actual-lead-time(currentSpeed) <
		// maximum-lead-time
		...
		if (router.getAppMode() == ApplicationMode.PEDESTRIAN) {
			...
			if ( (company != null) && (company.getId() != null) ) {
				...
				final TabletConfigs configs = configsDao.getByCompanyId(company.getId());
				if (configs == null) {
					TURN_DISTANCE = 50;
					return;
				}
				TURN_DISTANCE = configs.getTurnDistance(); // 7 sec
				if (Session.getDistanceUnit().equals(Company.MILES))
					TURN_DISTANCE = (int) (configs.getTurnDistance() * 3.2);
			}
		}
	}
	...
	/**
	 * if distance is less then default distance return true
	 * 
	 * @param dist
	 * @param etalon
	 * @return
	 */
	protected boolean isDistanceLess(double dist, double etalon) {
		if (dist < etalon)
			return true;
	...
	private void nextStatusAfter(int previousStatus) {
		// STATUS_UNKNOWN=0 -> STATUS_LONG_PREPARE=1 -> STATUS_PREPARE=2 -> STATUS_TURN_IN=3 ->
		// STATUS_TURN=4 -> STATUS_TOLD=5
	...
	protected void makeUTStatus() {
		// Mechanism via STATUS_UTWP_TOLD: Until turn in the right direction, or route is
		// re-calculated in forward direction
	...
	protected void updateStatus(Location currentLocation) {
		...
		if (nextInfo == null || nextInfo.directionInfo == null || nextInfo.directionInfo.distance == 0) {
			// if(currentStatus <= STATUS_UNKNOWN && currentDirection > 0){ This caused this prompt
			// to be suppressed when coming back from a
			// UTwp situation
		...
		if (currentStatus == STATUS_UNKNOWN) {
			...
			// if (dist >= PREPARE_LONG_DISTANCE && !isDistanceLess(speed, dist,
		...
		if (statusNotPassed(STATUS_TURN) && isDistanceLess(dist, TURN_DISTANCE)) {
	...
	class NextLocations {
		Location next1 = null, next2 = null;

		void calculate() {
			NextDirectionInfo nextInfo = router.getNextRouteDirectionInfo(new NextDirectionInfo(), true);
			if (nextInfo == null)
				return;
			NextDirectionInfo nextNextInfo = null;
			NextDirectionInfo nextNextNextInfo = null;
			nextNextInfo = router.getNextRouteDirectionInfoAfter(nextInfo, new NextDirectionInfo(), false);
			if (nextNextInfo != null)
				nextNextNextInfo = router.getNextRouteDirectionInfoAfter(nextNextInfo, new NextDirectionInfo(), false);
			try {
				if (nextNextInfo != null && nextNextInfo.directionInfo != null) {
					next1 = router.getRoute().getImmutableLocations().get(nextNextInfo.directionInfo.routePointOffset);
				}
				if (nextNextNextInfo != null && nextNextNextInfo.directionInfo != null) {
					next2 = router.getRoute().getImmutableLocations().get(nextNextNextInfo.directionInfo.routePointOffset);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if (play != null) {
			...
			if (pronounceNextNext != null) {
				...
				if (TurnType.TL.equals(t.getValue()) || TurnType.TSHL.equals(t.getValue()) || TurnType.TSLL.equals(t.getValue()) || TurnType.TU.equals(t.getValue())
					...
					play.then().bearLeft();
				} else if (TurnType.TR.equals(t.getValue()) || TurnType.TSHR.equals(t.getValue()) || TurnType.TSLR.equals(t.getValue()) || TurnType.KR.equals(t.getValue())) {
					play.then().bearRight();
				}
			}

			NextLocations nextLocations = new NextLocations();
			nextLocations.calculate();
			if (isPlay) {
				if (player instanceof TTSCommandStreetNamePlayerImpl) {
					((TTSCommandStreetNamePlayerImpl) player).playCommands(play, nextLocations.next1, nextLocations.next2);
				} else
					play.play();
			}
		}
	...
	public void newRouteIsCalculated(boolean newRoute) {
		...
		if (play != null) {
			if (!newRoute) {
				...
				// suppress "route recalculated" prompt for 60sec (this workaround now outdated
				// after more intelligent route recalculation and directional voice prompt
				// suppression)
