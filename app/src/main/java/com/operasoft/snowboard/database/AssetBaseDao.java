...
public class AssetBaseDao <T extends Dto> extends Dao<T> {
	
	public AssetBaseDao(String tablename) {
		super(tablename);
	}
	...
	public void replaceWorkorderId(String id, String newId) {
		String sql = "UPDATE " + table + " SET asset_id = '" + newId + "' WHERE asset_id = '" + id + "';";
		DataBaseHelper.getDataBase().execSQL(sql);
	}

	public void deleteListAttachedWithWorkorder(String wid) {
		final String sql = "delete  FROM " + table + " where asset_id = '" + wid + "'";
		Cursor cursor = null;
		try {
			DataBaseHelper.getDataBase().execSQL(sql);
		} catch (Exception e) {
	...
	public ArrayList<T> getListAttachedWithWorkorder(String workorderId) {
		...
		final String sql = "SELECT * FROM " + table + " where asset_id = '" + workorderId + "'";
		Cursor cursor = null;
		try {
